...
async function login({ email, password, role, ipAddress, userAgent }) {
  ...
  // ("disabled for local testing / developer convenience"), even though
  // registerFailedLoginAttempt/clearFailedLoginAttempts and the
  // account-locked email were all fully implemented and wired below.
  // Re-enabled as originally written — no logic changes, just turning
  // back on protection that already existed and was tested.
  if (user.lock_until && new Date(user.lock_until).getTime() > Date.now()) {
