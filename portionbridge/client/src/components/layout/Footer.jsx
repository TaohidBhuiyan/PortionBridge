    ...
    <footer style={{ background: PRIMARY_DEEPER }} className="text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10 pt-14 pb-10">
        <div className="grid md:grid-cols-[1.2fr_1fr_1fr] gap-10 pb-10">
          ...
          <div>
            <div className="font-mono text-[11px] text-white/35 mb-4">NAVIGATE</div>
            <div className="flex flex-col gap-2 text-sm text-white/70">
              <Link to="/#roles" className="hover:text-white transition-colors">Roles</Link>
              <Link to="/#leaderboard" className="hover:text-white transition-colors">Leaderboard</Link>
            </div>
          </div>
          <div className="md:text-right">
            <div className="font-mono text-[11px] text-white/35 mb-4">REACH US</div>
        ...
        <div className="border-t border-dashed border-white/15 pt-6 flex flex-wrap items-center justify-between gap-5">
          <div className="flex items-center gap-6 text-sm">
            <button onClick={() => onGoToRole("donor")} className="relative text-white font-medium">
              Donors
              <span className="absolute -bottom-2 left-0 w-1 h-1 rounded-full" style={{ background: PRIMARY }} />
            </button>
            <button onClick={() => onGoToRole("volunteer")} className="text-white/60 hover:text-white transition-colors">
              Volunteers
            </button>
          </div>
          <button
            onClick={() => onGoToRole("donor")}
            className="inline-flex items-center gap-2 bg-white text-sm font-medium px-5 py-2.5 rounded-full transition-transform hover:scale-105"
