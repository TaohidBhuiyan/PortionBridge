...
const LocationPermission = ({ onLocationGranted, onLocationDenied, onLocationBlocked }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-surface rounded-xl shadow-pb-modal max-w-md w-full p-6 relative border border-border">
        <button
          onClick={handleClose}
        ...
        <div className="flex flex-col items-center text-center">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${
            permissionState === 'unsupported' ? 'bg-danger-soft' :
            permissionState === 'blocked' ? 'bg-warning-soft' :
            ...
            {permissionState === 'unsupported' ? (
              ...
              <MapPin className="w-8 h-8 text-dash-primary" />
            )}
          </div>
          ...
          {error && (
            <div className="w-full bg-danger-soft border border-danger/20 rounded-lg p-3 mb-4">
          ...
          <div className="flex gap-3 w-full">
            {permissionState === 'prompt' && (
              <>
                <button
                  onClick={handleClose}
                  className="flex-1 px-4 py-2.5 border border-border rounded-lg text-text-primary hover:bg-surface-hover transition-colors font-medium"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isLoading}
                  className="flex-1 px-4 py-2.5 bg-dash-primary hover:bg-dash-primary-hover disabled:opacity-60 text-white rounded-lg transition-colors font-medium flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
            ...
            {(permissionState === 'denied' || permissionState === 'blocked') && (
              <>
                <button
                  onClick={handleRetry}
                  className="flex-1 px-4 py-2.5 bg-dash-primary hover:bg-dash-primary-hover text-white rounded-lg transition-colors font-medium"
                >
                  Try Again
                </button>
                <button
